...
New capability — there is no `openspec/specs/installed-inventory/spec.md` yet, so this delta is
ADDED-only. Nothing is MODIFIED, REMOVED or RENAMED anywhere in this file.

Delta summary: **11 ADDED requirements / 34 scenarios.**

Scope: acquiring the installed snapshot, projecting it, joining it with the catalog, deriving
outdated / pinned / on-request state, classifying self-updating casks, composing installed-state
filters for browse, keeping the inventory fresh, and behaving correctly when brew is absent.

Binding inputs, already settled — they are stated here as facts, not open questions:

- Product decisions (user-confirmed 2026-08-02, Engram `sdd/m2-installed-inventory/product-decisions`):
  the default installed view is on-request only; auto-updating casks never badge as outdated; browse
  installed-state filters ship in this change, composed through this capability.
- Live probe (brew 6.0.14, Engram `sdd/m2-installed-inventory/cask-outdated-probe`): the
  `info --installed` snapshot already applies brew's auto-updates exclusion to a cask's `outdated`
  flag, and the same record carries both the catalog `version` and the `installed` string, so a
  "newer version exists" signal for a self-updating cask is derivable without a second invocation.

Deliberately not specified here: `package-search` is untouched. Its requirement "Filters answerable
from the catalog alone" — including "No filter MUST depend on installed, not-installed, or outdated
state" — remains correct and byte-identical; composition happens above the index, and the invariant
is re-asserted as a scenario below.
