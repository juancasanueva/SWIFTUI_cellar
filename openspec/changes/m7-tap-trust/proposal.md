...
| **R6** | **Refusal-prose drift**, and the **formula** wording is unmeasured | Med | One structural phrase (`"untrusted tap"`), design-owned; a miss degrades to `.failed` with the verbatim log — the fallback `MutationOutcome` already documents. Widening needs no design change |
