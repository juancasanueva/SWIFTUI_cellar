...
- [ ] **A0.1** Capture the pre-slice baseline with both runners and record the counts as distinct
...
- [ ] **A0.2** Cite U25 / U31 / U32 / `sign_update` stdin from the design *Probe addendum*. **Do not
      re-run them.** *deps: none; ~0 lines*

### A1 App category — the cheapest independently revertible step (pbxproj item 6)

- [ ] **A1.1 RED** Create `cellarTests/UpdateProjectFileTests.swift` with a **self-contained**
...
- [ ] **A1.2 RED** Create `cellarTests/BundleUpdateKeysTests.swift`, **T11**: `Bundle.main.infoDictionary`
...
- [ ] **A1.3 GREEN** `cellar.xcodeproj/project.pbxproj` item 6: insert the key in **both** blocks,
...
- [ ] **A1.4 GREEN-on-arrival** `appTargetConfigurationsAreIdenticalModuloName` (`:213-229`) must pass
      unchanged. Run `app_unit_command`. *~0 lines; deps: A1.3*

### A2 Sparkle SPM dependency (pbxproj items 1–5) + `Package.resolved`

- [ ] **A2.1 RED** `UpdateProjectFileTests`, **T12(b)**: exactly one `XCRemoteSwiftPackageReference`
...
- [ ] **A2.2 GREEN** Add pbxproj items 1–5 — **18 lines measured**, five hunks (1 `PBXBuildFile`,
...
- [ ] **A2.4** `xcodebuild build -project cellar.xcodeproj -scheme cellar -destination
...
- [ ] **A3.1 RED** `BundleUpdateKeysTests`, **T10**: `Bundle.main.infoDictionary["SUFeedURL"]` is
...
- [ ] **A3.2 RED** `BundleUpdateKeysTests`, **T22 (bundle half)**: `Resources/Cellar-Info.plist` parses
...
- [ ] **A3.3 GREEN (partial)** Create `Resources/Cellar-Info.plist` with the two keys, the real feed
...
- [ ] **A3.4 🛑 MAINTAINER CHECKPOINT 1–3 — STOP AND REPORT.** Apply pauses. Maintainer:
...
- [ ] **A3.5 🛑 MAINTAINER CHECKPOINT 4 — STOP.** Paste the printed 44-character base64 public key
...
- [ ] **A3.6 RED** Create `cellarTests/UpdateKeyMaterialTests.swift`, **T20** (DD-12): across the
...
- [ ] **A3.7** Run `app_unit_command`: T10, T11, T12, T20, T22(bundle) green.
...
- [ ] **A4.1 RED** Create `cellarTests/UpdatePackageManifestTests.swift`, **T24**: parse
...
- [ ] **A4.2 GREEN** `Packages/CellarCore/Package.swift`: `+1 .library(name: "Updates", targets:
...
- [ ] **A4.3 RED** Create `Packages/CellarCore/Tests/UpdatesTests/AppVersionTests.swift` — **T1**
...
- [ ] **A4.4 GREEN** Create `Packages/CellarCore/Sources/Updates/AppVersion.swift` —
...
- [ ] **A4.5 RED** Create `Tests/UpdatesTests/AppcastDocumentTests.swift` + hand-authored
...
- [ ] **A4.6 GREEN** Create `Sources/Updates/AppcastDocument.swift` — validator over XML text
...
- [ ] **A4.7 RED** Create `Tests/UpdatesTests/UpdateCheckPresentationTests.swift` — **T6**: `nil` ⇒
...
- [ ] **A4.8 GREEN** Create `Sources/Updates/UpdateCheckPresentation.swift`. *~30–45 lines; deps: A4.7*
- [ ] **A4.9 RED** Create `Tests/UpdatesTests/AppUpdatingTests.swift` with `FakeAppUpdater` (an
...
- [ ] **A4.10 GREEN** Create `Sources/Updates/AppUpdating.swift` —
      `@MainActor public protocol AppUpdating: AnyObject, Observable` (DD-2) with the four members.
      *~25–40 lines; deps: A4.9*
- [ ] **A4.11 RED** Create `Tests/UpdatesTests/UpdatePolicyTests.swift` — **T7b**
...
- [ ] **A4.12 GREEN** Create `Sources/Updates/UpdatePolicy.swift` (DD-15) —
...
- [ ] **A4.13** Run `core_package_command`. All `UpdatesTests` green; the pre-slice CellarCore count is
...
- [ ] **A5.1 RED** `UpdateProjectFileTests`, **T12(c)**: one `PBXBuildFile` `Updates in Frameworks`,
...
- [ ] **A5.2 GREEN** Add pbxproj items 8–10 (≈6 lines). Cumulative pbxproj diff ≈**28 insertions, 0
...
- [ ] **A6.1 RED** Create `cellarTests/AutomaticUpdateChecksTests.swift`, **T13**: against a scratch
...
- [ ] **A6.2 GREEN** Create `cellar/Updates/AutomaticUpdateChecks.swift` (DD-5) — value type over an
...
- [ ] **A6.3 RED** Create `cellarTests/UpdateCompositionTests.swift` over `AppSecuritySources.load()`
...
- [ ] **A6.4 GREEN** Create `cellar/Updates/SparkleUpdateChecker.swift` — the **only** `import Sparkle`
...
- [ ] **A6.5 RED** `UpdateCompositionTests`, **T23**: `cellarApp.swift`'s `.commands { … }` body names
...
- [ ] **A6.6 GREEN** Create `cellar/Updates/CheckForUpdatesCommands.swift` over `let updater: any
...
- [ ] **A6.7 RED** `UpdateCompositionTests`, **T25 — added at this phase.** The design's
...
- [ ] **A6.8 GREEN** Create `cellar/Updates/UpdatesSettingsGroup.swift` (the `"Updates"` card, DD-3
...
- [ ] **A6.9 GREEN** `cellar/cellarApp.swift`: in `init`, after the release-notes block (`:313-344`),
      ...
      deps: A6.8*
- [ ] **A6.10 GREEN** `cellar/AppTestFixtures.swift`: `isUpdatesEnabled` for `--ui-testing-m6-updates`
...
- [ ] **A6.11** Run `app_unit_command` and `xcodebuild build …`. All new suites green;
...
- [ ] **A7.1 GREEN** `THIRD-PARTY.md`: add `## Sparkle` — project URL, version **2.9.6**, Sparkle
...
- [ ] **A7.2 GREEN** `RELEASING.md` §7: delete the `LSApplicationCategoryType` known-follow-up at
      ...
      *~4 lines; deps: A7.1*
- [ ] **A7.3** Full 3a suite: `app_unit_command` and `core_package_command`. Accepted: 0 failures,
...
- [ ] **A7.4** Confirm the final `project.pbxproj` diff is **exactly the ten items, ≈28 insertions, 0
...
- [ ] **B1.1 RED** Create `cellarTests/AppcastScriptContractTests.swift`, **T18**: `scripts/appcast.sh`
...
- [ ] **B1.2 RED** `AppcastScriptContractTests`, **T19** (DD-14's textual bridge, honest about being
...
- [ ] **B1.3 GREEN** Create `scripts/appcast.sh`, `chmod +x`. Six phases per design: **guard**
...
- [ ] **B1.4 ci-gate** — command: `shellcheck scripts/appcast.sh`. Accepted: **no findings, exit 0**.
      Not a test task. *~0 lines; deps: B1.3*

### B2 `release.yml` — extension point, permissions, the seventh secret

- [ ] **B2.1 RED** Create `cellarTests/AppcastWorkflowTests.swift`, **T16**: splitting the workflow on
...
- [ ] **B2.2 RED** `AppcastWorkflowTests`, **T17**: the `release` job declares `pages: write`,
...
- [ ] **B2.3 RED** Amend `cellarTests/ReleasePipelineCompositionTests.swift`
...
- [ ] **B2.4 RED** Amend `secretsAppearOnlyAsEnvironmentBindings` (`:668-692`) non-vacuity floor
...
- [ ] **B2.5 GREEN** `.github/workflows/release.yml`: insert the four steps at the named extension
...
- [ ] **B2.6 GREEN-on-arrival** `theWorkflowCanOnlyEverCreateARelease` (`:733-748`) must pass
      ...
      *~0 lines; deps: B2.5*
- [ ] **B2.7 ci-gate** — command: `actionlint .github/workflows/release.yml`. Accepted: **no output,
      exit 0**. Not a test task. *~0 lines; deps: B2.6*

### B3 Documentation

- [ ] **B3.1 GREEN** `RELEASING.md` §2: add prerequisites 6–8 — enable Pages with **source = GitHub
...
- [ ] **B3.2 GREEN** `PRD.md` :9 (Distribution line — Sparkle no longer "pending"), :124 (the update
...
- [ ] **B3.3 GREEN** `README.md`: state that the app updates itself via Sparkle from the published
...
- [ ] **B3.4** Run `app_unit_command`. All 3b suites green; the 3a suites unchanged. *~0 lines;
      deps: B3.3*
...
- [ ] **B4.1 🛑 MAINTAINER CHECKPOINT 5 — STOP.** Repository → Settings → Secrets →
...
- [ ] **B4.2 🛑 MAINTAINER CHECKPOINT 6 — STOP.** Repository → Settings → Pages → **Source: GitHub
...
- [ ] **B4.3 manual-evidence M1 / `U30` — the FIRST verification step of this phase.** Run
...
- [ ] **B4.4 manual-evidence M2 / M3 — re-checks on the *exported* bundle.** M2:
