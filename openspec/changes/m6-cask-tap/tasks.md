...
- [ ] P1 **Explicit confirmation to create a public repository** (`R11`, outward-facing). The pipeline
      MUST NOT take this action unilaterally. Phases 3–5 and the PR do not depend on it.
- [ ] P2 A Mac with Homebrew 6.0.18 for the two `manual-evidence` transcripts (S1, S3).
- [ ] P3 v1.0.0 is the latest **stable** published release, and its asset digest is still
...
- [ ] 0.1 Record the green baseline to compare against: `cellarTests` **232 passed / 0 failed** and
...
- [ ] 0.2 Confirm the four design anchors are still where the design says (they were at `bcb9d6b`):
...
- [ ] 1.1 **GATE — P1.** Do not proceed without the maintainer's explicit confirmation. Creating a
      public repository is outward-facing and irreversible in effect (R11).
- [ ] 1.2 `gh repo create juancasanueva/homebrew-cellar --public --description "Homebrew tap for Cellar"`.
...
- [ ] 2.1 Commit the four files plus `LICENSE` in one commit: `Casks/home-cellar.rb`,
...
- [ ] 2.2 Copy the cask from `design.md` **without re-deriving** `version`/`sha256`: `1.0.0` and
...
- [ ] 2.3 `ci.yml` MUST register the checkout with `cp -R` into
...
- [ ] 2.4 **`ci-gate` — S2.** Commands, in `ci.yml` on `macos-26`: `brew style juancasanueva/cellar`;
...
- [ ] 2.5 **`ci-gate` — S4.** Run `bump.yml` via `workflow_dispatch`. **Accepted evidence**: the log
...
- [ ] 2.6 **`ci-gate` — S5.** Run `bump.yml` **twice** against an unchanged `releases/latest`.
...
- [ ] 3.1 **RED scaffold** Create `cellarTests/CaskZapInventoryTests.swift` with a **self-contained**
      ...
      Support` does).
- [ ] 3.2 **RED — T1** `everyWriteRootTheSourceDeclaresIsInTheDocumentedInventory` (**S6**). Implement
...
- [ ] 3.3 **RED — T2** `theWriteRootScanIsNonVacuousAndEveryPassThroughIsAHomebrewRootsHandOff`
...
- [ ] 3.4 **RED — T3** `theRunbookNamesBothKeychainItemsAZapCannotRemove` (**S7**). `RELEASING.md`
...
- [ ] 3.5 **RED — T4** `theReadmeCarriesBothBrewCommandsAsWholeLines` (**S8**). Some line of
...
- [ ] 3.6 **RED — T5** `theWorkflowGainsNoCrossRepositoryReach` (**S9**) — added to the existing
...
- [ ] 3.7 Run the narrow command; **all five MUST fail, each for the stated reason**. A test that is
...
- [ ] 4.1 **GREEN — T3 + T1/T2 half.** `RELEASING.md`: insert the new **§8 "The Homebrew tap"** before
...
- [ ] 4.2 **GREEN — DD-12.** `RELEASING.md` §7 (`:309-311`): replace the inherited-contract paragraph
...
- [ ] 4.3 **GREEN — T4.** `README.md` `## Install` (`:32`, above `## Updates` at `:41`): replace with
...
- [ ] 4.4 **GREEN — T5.** `.github/workflows/release.yml`: delete **exactly** lines 175-176 (the
...
- [ ] 4.5 **GREEN — DD-13.** Create top-level `LICENSE`: the standard MIT text with
...
- [ ] 4.6 **GREEN — docs.** `PRD.md` `:9` (Distribution cell), `:194` (Cask channel bullet →
...
- [ ] 4.7 Run `… -only-testing:cellarTests`; **T1–T5 green**. Commit WU3:
...
- [ ] 5.1 Full suite: `xcodebuild test -project cellar.xcodeproj -scheme cellar -destination 'platform=macOS,arch=arm64' -only-testing:cellarTests`
      → **232 + the 5 new cases, 0 failures**, counted as distinct `Test case '…' passed` ids. Assert
      counts, never `TEST SUCCEEDED` alone.
- [ ] 5.2 `swift test --package-path Packages/CellarCore` → **1753 passed / 209 skipped**, unchanged.
      This is the 0-line-diff proof, not a coverage claim.
- [ ] 5.3 **Bindings proof.** `git diff --stat main -- scripts/ cellar.xcodeproj/project.pbxproj Packages/CellarCore cellar/ cellar.xcodeproj/xcshareddata`
      → **empty output**. Any line here is a deviation to report before merge, not to absorb.
- [ ] 5.4 Re-read `RELEASING.md` §8 end to end as a runbook and copy-paste both `README.md` install
...
- [ ] 6.1 `ci.yml` run URL + exit status for style, both audits, install, zap → **S2** (from 2.4).
- [ ] 6.2 `bump.yml` run twice against an unchanged `releases/latest`, zero commits the second time →
      **S5** (from 2.6).
- [ ] 6.3 `bump.yml` `workflow_dispatch` log showing the stable tag resolution or the prerelease
...
- [ ] 6.5 Verbatim `brew upgrade` transcript against a Sparkle-self-updated copy showing no reinstall →
      **S3** (from 2.8).
- [ ] 6.6 Archive obligation, recorded now so it is not re-derived: the main spec's
