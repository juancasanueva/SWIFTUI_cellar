...
- [x] 0.1 `brew install actionlint shellcheck` — local tooling for G1/G2. Not a repo change.
- [x] 0.2 Cite the U29 baseline (CellarCore **1732/1732**, `cellarTests` **141 distinct** cases, 0
...
- [x] 1.1 **RED** Create `cellarTests/ReleasePipelineCompositionTests.swift` with its **self-contained**
...
- [x] 1.2 **GREEN** `cellar.xcodeproj/project.pbxproj`: add `ARCHS = arm64;` as the first `buildSettings`
...
- [x] 1.3 **GREEN-on-arrival** (declared, not RED) in `ReleaseMetadataTests`: **T14** the two app-target
...
- [x] 1.4 Confirm the diff is exactly two insertions: `git diff --stat cellar.xcodeproj/project.pbxproj`
      → `1 file changed, 2 insertions(+)`. Any other line is a deviation to report, not to absorb.

## Phase 2: Copyright — Work unit 2 (S28, S29)

- [x] 2.1 **RED** **T9** in `ReleaseMetadataTests`: `cellar/InfoPlist.xcstrings` parses as JSON and
...
- [x] 2.2 **RED** **T10** in `ReleaseMetadataTests`:
...
- [x] 2.3 **GREEN** `cellar/InfoPlist.xcstrings`: set `NSHumanReadableCopyright` to the exact string and
...
- [x] 2.4 Re-run `fast_unit_command`; T9 + T10 green; re-confirm the pbxproj diff is still 2 lines.

## Phase 3: Release script and export options — Work unit 3 (S20, S21, S22)

- [x] 3.1 **RED** suite `ReleasePipelinePlacementTests` — **T1**: `scripts/ExportOptions.plist` exists;
...
- [x] 3.2 **RED** suite `ReleaseWorkflowContractTests` — **T7**: `ExportOptions.plist` parses via
      `PropertyListSerialization` with `method == "developer-id"`, `signingStyle == "automatic"`,
      `teamID == "Z3S5JK8E38"` (S22).
- [x] 3.3 **RED** **T8**: `release.sh` contains every stage command — `xcodebuild archive`,
...
- [x] 3.4 **RED** **T15a**: `release.sh` contains **no `gh `** and **no `git `** at all — it can neither
...
- [x] 3.5 **RED** **T16a**: `release.sh` contains no `set -x` and no `set -eux` (S26).
      *Threat row: secret exposure.*
- [x] 3.6 **GREEN** Create `scripts/ExportOptions.plist` with the design's complete content
      (`developer-id` / `automatic` / `Z3S5JK8E38` / `destination export` /
      `manageAppVersionAndBuildNumber false`).
- [x] 3.7 **GREEN** Create `scripts/release.sh` per design: `archive export package notarize staple
...
- [x] 3.8 **ci-gate G2** — command: `shellcheck scripts/release.sh`. Accepted output: **no findings,
      exit 0**. Not a test task.

## Phase 4: Release workflow — Work unit 4 (S3, S4, S19, S21, S24, S25, S26)

- [x] 4.1 **RED** **T2** in `ReleasePipelinePlacementTests`: `.github/workflows/release.yml` exists (S21).
- [x] 4.2 **RED** **T3**: positives — the only trigger is `push:` → `tags:` containing `'v*'`; negatives —
...
- [x] 4.3 **RED** **T19**: a step whose body contains `github.event.repository.private` **and** `exit 1`
...
- [x] 4.4 **RED** **T4**: split the workflow on `- name:` step boundaries; the step whose body contains
      `security delete-keychain` **also** contains `if: always()` (position-independent, not a bare
      substring pair) (S25).
- [x] 4.5 **RED** **T5**: every `${{ secrets.X }}` occurrence sits on a line matching
      `^[A-Z0-9_]+: \$\{\{ secrets\.[A-Z0-9_]+ \}\}$` — only as `env:` right-hand sides (S26).
      *Threat row: secret exposure.*
- [x] 4.6 **RED** **T6**: the set of referenced secret names **equals** the six expected names (set
      equality, so a seventh secret fails too) (S26).
- [x] 4.7 **RED** **T15b**: the workflow contains no `gh release delete`, `gh release edit`, `git push`,
      `git tag`, `git commit`, `git add`; the only `gh ` invocation is `gh release create` (S18, S19).
      *Threat row: PR commands.*
- [x] 4.8 **RED** **T16b**: the workflow contains no `set -x` and no `set -eux` (S26).
- [x] 4.9 **GREEN** Create `.github/workflows/release.yml` exactly per the design's YAML: private-repo
...
- [x] 4.10 **GREEN-on-arrival** (declared, not RED, and authored in this commit so the pin predates the
...
- [x] 4.11 **ci-gate G1** — command: `actionlint .github/workflows/release.yml`. Accepted output:
      **no output, exit 0**. Not a test task.

## Phase 5: Documentation — Work unit 5 (S8, S12)

- [x] 5.1 **RED** **T17b** in `ReleaseMetadataTests`: `RELEASING.md` contains the literal `1.0.0 (1)`
...
- [x] 5.2 **RED** **T18**: `RELEASING.md` names all five literally — `allow-jit`,
      `allow-unsigned-executable-memory`, `disable-library-validation`, `ENABLE_USER_SELECTED_FILES`,
      `REGISTER_APP_GROUPS` (S12).
- [x] 5.3 **GREEN** Create top-level `RELEASING.md` §1–§8 per the design table: CI is the only publishing
...
- [x] 5.4 **GREEN** `PRD.md` :9, :157, :168, :187, :212, :224, :227 — rewritten in place with the reason
...
- [x] 5.5 **GREEN** `README.md`: `## Install` between `## Requirements` and `## Building` (download,
...
- [x] 5.6 **ci-gate G4** — commands: `xcodebuild test -project cellar.xcodeproj -scheme cellar
...
> **Apply status (2026-08-22): every Phase 6 item below is blocked on maintainer prerequisites P2/P3.**
> Re-measured at apply time: `security find-identity -v -p codesigning` still lists exactly one identity,
> `Apple Development: Juan Casanueva (A8EB4839B9)` — no Developer ID Application certificate and no App
> Store Connect API key exist on this machine, so `U26` cannot run, `U28` and `G3` follow it, and the
> Manual-signing fallback (6.3) must not be applied before `U26` has a measured result. 6.8 is optional
> hardening that the design records as "none required".
