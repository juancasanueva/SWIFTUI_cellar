...
| **DD-5** | The revocation is **unconditional** — including for an `unreported` tap — and its failure does not block the removal | Gate the untap on a successful untrust; skip the untrust when the state is not `trusted` | TM7 :221-231 states both halves as rules. `brew untrust` on a never-trusted tap exits 0 (obs `#7722`), so no precondition exists to check; blocking removal on a failed revoke would leave the user unable to remove the tap at all. **Homebrew < 6 consequence, stated rather than discovered:** a brew with no `untrust` verb makes *every* untap show a visibly failed `untrust` item in Activity beside a succeeding `untap`. That is the spec's own choice (TM7 :228-231: the failure "MUST appear as its own visible operation … rather than being swallowed"), not a defect — see R5. **Amended by D4 (2026-08-23) — see DD-5a** |
| **DD-5a** *(D4, 2026-08-23 — post-verify correction)* | The sequence is **reversed and made dependent**: `[.removeTap, .untrustTap]`, and the revocation is submitted only once the removal settles with `MutationOutcome.succeeded`. A refused removal submits none of it | Keep DD-5's order and simply tolerate the refusal; submit the revocation anyway and let it fail beside the refused removal; hide Untap while untrusted | Measured on Homebrew 6.0.18: `brew untap` on a tap with installed packages exits 1 with `Refusing to untap … because it contains the following installed casks: …`, and `brew untrust` after `brew untap` exits 0 and removes the grant — so reversing is safe and gives up nothing. Under DD-5's order the user reached a state with no exit: grant revoked, tap still installed, and Force Untap hidden because DD-14 offers it only for a tap whose packages Homebrew will still name, while the footer pointed them back at the Untap that had just failed. TM13.6 is preserved exactly — the grant still never outlives a *successful* removal — and PM3 is untouched, because the confirmation still covers the whole sequence all-or-nothing. **This does not weaken DD-5's unconditionality:** behind an accepted removal the revocation still runs whatever the reported trust state, for the same obs #7722 reason. The new seam is `OperationCenter.submitDependentSequence`, beside `submitSequence` rather than replacing it: an N-package fan-out must keep failing independently |
...
## Data Flow
    ...
    Untap      ──► [.removeTap, .untrustTap]      ──► submitDependentSequence ─► no sheet (neither confirms)
    Force Untap──► [.forceRemoveTap, .untrustTap] ──► submitDependentSequence ─► ONE sheet, leadDisclosure
                                                                        leads with force ⇒ .forceUntap
      (D4, 2026-08-23) the trailing untrust is submitted ONLY after the removal settles .succeeded;
      a removal brew refuses submits no untrust and leaves exactly one failed item carrying its reason.
            │
            ▼  every terminal
...
**D4, 2026-08-23 (post-verify correction — DD-5a).** `submitSequence` stays exactly as above for every
other caller. Untap and Force Untap move to a sibling with the same confirmation shape and a different
execution rule:
...
`ConfirmationRequest` gains `dependsOnLead: Bool = false`, read by `confirm(_:)` alone — the sheet, the
listed argv and the all-or-nothing rule are identical either way. `ActivityItem` gains an internal
`onSettle(_:)` whose handlers `settle(_:)` drains and runs once; `settle` is already guarded once-only
by `OperationCenter.finish`, so no second guard is added. Followers are armed recursively — each one
arms the next when it settles — and only on `outcome.isSuccess`, so cancelled, refused, launch-failed
and abandoned leads all stop the sequence.
