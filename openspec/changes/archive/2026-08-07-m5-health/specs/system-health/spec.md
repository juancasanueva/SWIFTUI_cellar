...
New capability — there is no `openspec/specs/system-health/spec.md` yet, so this delta is
**ADDED-only**: **11 requirements / 51 scenarios**. Nothing is MODIFIED, REMOVED or RENAMED.
...
Traceability: **D1/D2** are `installed-inventory` and `local-package-metadata` deltas, not this file.
**D3** → "The score is computed over answered inputs only, and its unknowns are inseparable from it"
and "Every weight is visible in the breakdown". **D4** — the tenth `AppSection` case, the retention of
Home as the landing section, and the resolution of the deferred question at
`cellar/Shell/AppSection.swift:17–20` — is an **app-side placement decision with no `CellarCore`
behaviour**, so it is carried by this change's `tasks.md` and deliberately **not** by a requirement
here, exactly as `brewfile-management` carried its own D3 and `release-notes` its D4. Recorded so a
later reader does not read its absence as a gap: the decision is "keep Home", and the source's
deferred question must be marked resolved rather than deleted. **D5/D6** → "A non-zero doctor exit is
an ordinary outcome, and the document arrives on stderr" and "Doctor evidence preserves every byte and
counts every line it cannot group" — raw **and** grouped, which is the only reading that satisfies
both PRD §3.4 and §9 Q5. **D7** → "The doctor inversion is quarantined to this capability", plus the
Provenance note below; `openspec/specs/brew-execution/spec.md` is deliberately **not** modified by this
change and is expected to show a 0-line diff.
