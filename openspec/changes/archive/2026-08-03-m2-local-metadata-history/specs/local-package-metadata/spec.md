...
New capability — there is no `openspec/specs/local-package-metadata/spec.md` yet. Delta summary:
...
offered version revives it. Owned by `Packages/CellarCore`.
...
- **Snooze is "until next version" only.** PRD §3.2 also lists 1 day / 1 week / 1 month; those are
  deliberately narrowed out of this slice, so no duration and no clock dependency exists.
- **Revival is version inequality, not version ordering** (G5 ruling, user-confirmed, Engram
  `#7117`). Homebrew version strings — `1.2.3_1`, date-based, tap-specific — cannot be ordered
  ...
  No Homebrew version comparator exists in this capability.
