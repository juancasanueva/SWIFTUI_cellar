# Delta for release-notes

New capability — there is no `openspec/specs/release-notes/spec.md` yet, so this delta is
**ADDED-only**: **9 requirements / 39 scenarios**. Nothing is MODIFIED, REMOVED or RENAMED.
...
Traceability: **D1** → "Nothing leaves this Mac before a dated release-notes grant"; **D2** → the
same requirement's distinct-service and distinct-grant clauses; **D3** → "A two-tier TTL cache,
keyed by repository and version"; **D4** is an app-side entry-point decision with no CellarCore
behaviour and is carried by `tasks`, not by a requirement here; **D5** → "The token is a Keychain
secret, and it is optional"; **D6** → "Resolution is a union of the published URLs, never a guess"
(union rule, tie-break order, and resolvability answerable without egress so an entry point can be
gated on it).
