...
**Round 7 (2026-08-25 maintainer product decision).** **DD-23** is new and supersedes the **collision**
half of **DD-21**: `routableID` becomes `unique` alone, so a colliding hit is routable and lands on the
catalog's own detail through the branch order **DD-21** already asserted. It is the cheapest decision in
the change — **one deleted conjunct**, no new type, no new branch, no new file — and the reason is that
**DD-21** already put the destination in place. `PackageDetailView.swift` returns to a **zero diff for
this round**: it changed in round 6 and does not change again, which the bindings proof states as
"unchanged since `6f18d2d`" rather than "byte-identical to `main`". `TapSearchView.swift` changes by a
**comment only** — the inert-row comment **DD-21** restated names the collision as a bar, and that is
now false. **DD-8** is unaffected for the sixth round running; **DD-18**, **DD-19**, **DD-20** and
**DD-22** are untouched.
...
**Round 8 (2026-08-25 maintainer UI feedback).** **DD-24** is new and supersedes the **mutation-surface**
clause of **DD-22** and, for the receipt pane, m10's own DD-8: both tap-backed panes stop hanging
`MutationMenu` in the header's `primaryButton` slot and instead render the **catalog pane's own Actions
section**, at the foot of the pane. It is the third time this change has met the same problem — a
component the catalog surface already declares, `private` to a file the extensions cannot reach — and
the third time the answer is **DD-18**'s: relax exactly what the extensions need and share the one
declaration. `MutationMenu.swift` is untouched for the fourth round running and stays on the **rows**.
What moves is `PackageDetailView.swift`, whose diff was already open from round 6 and now carries a
**behaviour-preserving refactor**: `actionsSection` and `primaryCommand` are rebuilt from the
`PackageEntry` the catalog pane already derives, because that is every input either of them ever read.

**Round 9 (2026-08-25 maintainer UI feedback, final).** **DD-25** is new and **extends DD-9**'s row
composition with its last missing element: the tap row draws `PackageIconTile`, the same leading tile
`PackageRow` and `InstalledRow` draw, with the same argument shape. It is the fourth time this change
has answered the same instruction — chip (r2), pill (r3), update pill (r4), verbs (r5) — and the fourth
time the answer is **DD-18**'s: reference the one shipped component rather than compose a lookalike.
`PackageIconTile` needs **no edit**: its cask branch already degrades to the coloured initial tile for a
token the artwork registries do not carry, which is precisely a third-party tap's cask, and its formula
branch already draws the bundled glyph. What moves is two optional properties on `TapSearchView`, the
one `ContentView` call site that fills them, and one nested `HStack` in the row. **DD-12** survives
intact: no `.task`, no `await`, no `async` joins this file, so the surface still starts nothing.
...
| **DD-4** (install-state clause superseded r6 by **DD-21**; collision clause superseded r7 by **DD-23**) | Selectable **iff** `routableID != nil` — ~~nil when not installed~~, ~~when `alsoInCatalog`~~, or when another emitted hit shares the `PackageID`. Selection stays `PackageID?`, and that half is untouched by both supersessions | `:64-67`, `:152-172` |
...
| **DD-23** (new, r7) | `routableID` becomes **`unique`** alone — the `collides == false` conjunct is **deleted**, so the only hit that withholds its route is one whose `PackageID` another **emitted** hit also carries. `unique` keeps being counted over the emitted hits alone, by the `occurrences` fold that already exists; a catalog record is **not** an emitted hit and is not counted. `alsoInCatalog` and `collisionNote` are **unchanged**, still stored, still pinned. A colliding hit therefore hands over its exact bare `PackageID`, and `PackageDetailView.body`'s **first** branch — `catalog.package(id)`, unchanged since m1 — answers it | Adding a fourth branch, or a `routesToCatalog` flag on the hit, so the surface could say *where* a hit goes (the destination is the resolution order's answer, not a fact the projection can know — and a second opinion on it is the drift `DD-6`, `DD-7` and `DD-21` were each written to prevent); withdrawing the collision note now that the row opens the colliding package (the note answers a **mutation** question — which package an install from this row installs — and that answer is unchanged); keeping `collides` in the expression but negating it only for the not-installed case (two rules where one will do, and the install state is exactly what `DD-21` removed from this decision); routing a colliding hit to the **tap inventory** pane instead (it would present the tap's name-only facts for a package Homebrew will not install from that tap — the mismatch this round exists to remove, inverted) | **The destination was already correct; the withholding was the defect.** `DD-21` reasoned that a colliding row must not route because catalog-first resolution "would present a different package than the row chosen". That measured the pane against the *row*, when the row's own pinned note already tells the user Homebrew installs **the catalog package** — so the catalog pane is the package the row promises, and the old rule left that sentence pointing at something the user could not open. The change is a deleted conjunct because `DD-21` built the whole route already: the identity is the bare `PackageID`, the order resolves the catalog first, and both were asserted before this round. What remains inert is the case nothing can fix — two taps publishing one name, where every candidate destination is equally wrong — and it stays presented and installable, exactly as before |
| **DD-24** (new, r8) | The catalog pane's `actionsSection` is **rebuilt from a `PackageEntry`** rather than from a `CatalogPackage`, relaxed from `private` to `internal`, and **called** by both tap-backed panes as the last block of their scroll content; `primaryCommand(for:target:)` moves with it and reads `entry.installed` instead of re-querying `installed.inventory`. Both panes pass `EmptyView()` as `primaryButton`, and `MutationMenu` leaves both of them — it stays on the **rows**, unchanged. The refactor is provably behaviour-preserving because the entry is *every* input the section ever had: `PackageTarget(package.id)`, `FormulaID(package.id)`, `CaskID(package.id)`, `entry(for: package)` and `installed.inventory.package(package.id)` are all `entry.id` or `entry.installed`, and `entry.catalog` is read by nothing inside it. Only the `actionsSection` declaration is relaxed: `submit`, `quietButton`, `accentButton`, `dangerButton`, `snoozeButton` and `primaryCommand` stay `private`, because they are called from inside the same file | Copying the section into each pane (three declarations of one set of verbs — the exact drift **DD-18** was created to end, and the drift II15's clause exists to forbid); keeping `MutationMenu` in the header **and** adding the section (two places to act on one pane, which is the defect stated twice); extracting the section into a third file as `StatusPill` was extracted (its body calls six `private` helpers and reads four stored properties of the view, so extraction means moving all of them or passing all of them — a large diff on a file that is under a narrowed-diff claim, to reach a shape `internal` already reaches); giving the section a `CatalogPackage?` parameter so the tap panes could pass `nil` (an optional whose only use is to be `nil`, and the catalog record is exactly what PD6 forbids these panes to have); a protocol over “things with an id and an installed record” (`PackageEntry` **is** that type, and has been since the Installed list shipped) | **`PackageEntry` was already the right parameter; the signature was simply older than the type.** `actionsSection` never read a catalog field — it read an identity and an installed record, which is what `PackageEntry` carries — so this is a signature catching up with its body rather than a generalisation invented for the tap panes. That is what makes the round cheap and what makes “one component” literally true rather than aspirational: the catalog pane's call site becomes `actionsSection(for: entry(for: package))`, the value it already computed for `PackageMetadataSection` on the line above. The panes' entries are unchanged — `receiptEntry` still says `installed: snapshot, catalog: nil`, `publishedEntry` still says `installed: nil, catalog: nil` — so **DD-20**'s keying, PD6's no-synthesized-record rule and PM10's bare-token argv all hold without an edit. The header slot going empty is not a side effect but half the maintainer's instruction: one pane, one place to act |
| **DD-25** (new, r9) | The tap row's `HStack` gains a nested `HStack(spacing: 10)` whose first element is `PackageIconTile(id: entry.id, assets: assets, iconLoader: iconLoader)` — byte-for-byte `PackageRow.swift:34`'s call, over the same `PackageEntry` the row already builds for `MutationMenu`, so the tile and the menu can never disagree about which package the row is. `TapSearchView` gains `var assets: CaskBrowseAssets?` and `var iconLoader: CaskIconLoader?`, declared and ordered exactly as `BrowseView` declares them, and `ContentView`'s one `TapSearchView(` call site passes the same `caskAssets` / `caskIcons` it already passes to `BrowseView`. `PackageIconTile`, `CaskIconView`, `CaskIconLoader` and `CaskBrowseAssets` are **all unedited** | Composing a local `Image(systemName:)` or a local `PackageTile` on the row (a lookalike — the exact drift **DD-18** exists to end, and it would answer the formula case wrongly the moment the shared component's glyph changed); adding `.task { await assets?.load() }` as `BrowseView` does (it would put `await` and `.task` into a file PS8's process-layer scenario scans for exactly those tokens, and buy nothing: the manifest only gates the CaskFlow rungs, and a tap cask is never in it); passing the tile `hit.id` (a `RowID`, not a `PackageID` — **DD-2** made that unrepresentable on purpose); making the two properties non-optional (every other row that draws this tile declares them optional so previews render, and `BrowseView`'s preview relies on it) | **The component was already correct for this row; only the row had not asked it.** `PackageIconTile`'s cask branch hands the token to `CaskIconLoader`, whose ladder for an unknown token is App-Fair alone and whose miss answers `nil`, which `CaskIconView` renders as `PackageTile` — the coloured initial tile. That is the designed answer for a third-party tap's cask, not a degraded one, and it needed no fallback fix. The formula branch is unconditional. So the whole round is two properties, one call-site argument pair and one nesting level, and the tile is *provably* the same one Browse draws rather than one that merely looks like it |
...
### File Changes — round-7 delta (maintainer product decision, a colliding hit opens the catalog detail)

| File | Action | Est. delta | Description |
|---|---|---|---|
| `Packages/CellarCore/Sources/BrewClient/TapPackageSearch.swift` | Modify | +8 / −10 | **DD-23**: `routable` drops the collision conjunct — `let routable = unique` — and the comment above it is restated. `alsoInCatalog`, `collisionNote` and the `occurrences` fold are all untouched |
| `Packages/CellarCore/Tests/BrewClientTests/TapPackageSearchTests.swift` | Modify | +45 / −25 | The two rows that pinned the retired rule are **replaced, not narrowed**; the duplicate-only row and the unchanged colliding-receipt row stay |
| `cellar/Browse/TapSearchView.swift` | Modify | +5 / −7 | **Comment only.** The inert-row comment names the collision as a bar; that is now false. No behaviour line changes — the gate is still `hit.routableID` |
| `cellarTests/TapSearchCompositionTests.swift` | Modify | +25 / −12 | The selection-rule scan's recorded reason restated to duplicate-only; the forbidden-token list and the catalog-first order assertion both unchanged |
| `cellar/Browse/PackageDetailView.swift` | **Unchanged this round** | **0** | **DD-23** adds no branch: the catalog branch it already ships is the destination. The bindings proof asserts "unchanged since `6f18d2d`", not "byte-identical to `main`" |
| `Packages/CellarCore/Sources/BrewClient/TapInventoryDetail.swift`, `cellar/Browse/PackageDetailView+TapInventory.swift` | **Untouched** | **0** | The inventory branch answers only for an identity the catalog does not carry, which round 7 does not change |
| `cellar/Activity/MutationMenu.swift`, `cellar/Browse/PackageRow.swift`, `cellar/Browse/StatusPill.swift` | **Untouched** | **0** | No mark, no verb and no argv moves |
| `cellar/Browse/BrowseView.swift` | **Untouched** | **0** | **DD-8** holds for the sixth round running |
| `cellar.xcodeproj/project.pbxproj` | **Untouched** | **0** | No file is created this round |

**The production change is one expression.** Everything else in the table is a comment, a test or a
docs artifact. That is the honest measure of **DD-23**: round 6 built the identity, the branch and the
ordering, so round 7 only stops withholding what they already resolve correctly.

### RED rows — round 7

| Class | Test | What it proves |
|---|---|---|
| `unit` | `aCollidingHitIsShownAndIsRoutable` (**replaces** `aCollidingHitIsShownAndIsNotRoutable`) | **DD-23**: the colliding hit keeps `alsoInCatalog`, keeps the pinned note byte-for-byte, keeps its own `RowID` and its bare `mutationTarget` — **and** reports `routableID == mutationTarget`. Replaced rather than narrowed: the old row asserted exactly the `nil` the decision overturned |
| `unit` | `onlyADuplicatedIdentityWithholdsTheRoute` (**replaces** `anAmbiguousHitIsNotRoutableInEitherInstallState`) | **DD-23**: over the same five fixtures the round-6 row used, the two colliding hits flip from `nil` to their exact `PackageID` in **both** install states, the duplicate pair stays `nil`, and both uncollided positives are unchanged — so the decisive variable is uniqueness alone, triangulated against install state and against collision |
| `unit` | `twoTapsPublishingOneNameAreBothUnroutable` (**unchanged**) | **DD-23**'s surviving inert case, asserted by a row that needed no edit — which is itself the evidence that the rule narrowed rather than vanished |
| `unit` | `aCollidingCatalogReceiptIsNeverAttachedToATapRow` (**unchanged except its two routability lines**) | **DD-20** is untouched: the tap row still carries its **own** receipt or none, never the catalog's. Only the two `routableID == nil` assertions inside it move, because they pinned the retired rule |
| `unit` | `aCollidingSelectionResolvesToTheCatalogsOwnRecord` (new, **PD6**) | PD6's round-7 scenario: the record the ordinary catalog lookup returns for a colliding identity is byte-identical to the one it returns with **no tap inventory resident**, the index record count is the catalog's own, and catalog search for that query is unchanged |
| `unit-app` | `theTapSearchSurfaceSelectsOnRoutabilityAlone` (**unchanged subject, restated reason**) | **DD-23** costs the view nothing: selection was gated on `hit.routableID` alone before and after. The forbidden-token list — including `alsoInCatalog` — is **unchanged and now load-bearing in a second way**: a view that consulted the collision would now be wrong about real rows, not merely ill-mannered |
...
### File Changes — round-8 delta (maintainer UI feedback, the shared Actions section on both tap panes)

| File | Action | Est. delta | Description |
|---|---|---|---|
| `cellar/Browse/PackageDetailView.swift` | Modify | +14 / −14 | **DD-24**: `actionsSection(for:)` takes a `PackageEntry` and is `internal`; `primaryCommand(for:target:)` takes the same entry and reads `entry.installed`; the catalog call site becomes `actionsSection(for: entry(for: package))`. **No button, no label, no identifier, no argv and no ordering changes** |
| `cellar/Browse/PackageDetailView+Receipt.swift` | Modify | +6 / −8 | **DD-24**: `primaryButton` becomes `EmptyView()`, `MutationMenu` leaves, `actionsSection(for: receiptEntry(for: snapshot))` joins the scroll content **after** `receiptFooter` |
| `cellar/Browse/PackageDetailView+TapInventory.swift` | Modify | +6 / −8 | **DD-24**: the identical change, with `actionsSection(for: publishedEntry(for: published))` after `publishedFooter(published)` |
| `cellarTests/ReceiptDetailCompositionTests.swift` | Modify | +30 / −12 | II15's amended verb scenario: the pane calls the shared builder (pinned with its terminator), references no `MutationMenu(`, composes no verb literal, and its header slot is `EmptyView()` |
| `cellarTests/TapSearchCompositionTests.swift` | Modify | +30 / −4 | PS8's amended pane-composition scenario: the same four claims for the name-only pane |
| `openspec/changes/m11-tap-search/specs/installed-inventory/spec.md` | **Create** | ~250 | The MODIFIED **II15** block — whole block, 11 of 12 scenarios byte-identical |
| `cellar/Activity/MutationMenu.swift` | **Untouched** | **0** | It stays on the **rows**. Round 8 removes two call sites and edits no line of it |
| `cellar/Browse/TapSearchView.swift` | **Untouched** | **0** | The row's `⋯` menu is the affordance the maintainer kept |
| `cellar/Browse/BrowseView.swift` | **Untouched** | **0** | **DD-8** holds for the seventh round running |
| `Packages/CellarCore/**` | **Untouched** | **0** | No projection, no fact and no copy moves: round 8 is presentation only |
| `cellarTests/PerPackageTrustCompositionTests.swift` | **Untouched** | **0** | Both panes are already in `views()`; no file is created in the app target |
| `cellar.xcodeproj/project.pbxproj` | **Untouched** | **0** | No app-target file is created this round |

**`PackageDetailView.swift`'s diff widens, and the bindings claim widens with it.** Round 6 narrowed it
from "byte-identical to `main`" to "these two hunks"; round 7 left it untouched. Round 8 adds the
refactor, so the claim becomes "the round-6 hunks, plus the entry-shaped `actionsSection`/`primaryCommand`
and one relaxed `private`" — shown in full in the bindings proof, with the property that matters stated
positively: **every verb label, every accessibility identifier, every command family and the section's
internal order are byte-unchanged**, and the catalog pane's shipped tests are the proof.

### RED rows — round 8

| Class | Test | What it proves |
|---|---|---|
| `unit-app` | `theReceiptPaneOffersTheSameVerbsAsTheRow` → **`theReceiptPaneOffersTheCatalogPanesActionsSection`** (replaces) | II15's amended scenario: the pane calls `actionsSection(for: receiptEntry(for: snapshot))`, references no `MutationMenu(`, passes `EmptyView()` in the header slot, and composes none of the five verb literals — which live in `PackageDetailView.swift` alone, asserted positively there so the absence is not vacuous |
| `unit-app` | `theNameOnlyTapDetailComposesNothingItCannotKnow` (**amended in place**, item 4) | PS8's amended pane clause: the identical four claims for the name-only pane, with its `PackageEntry(installed: nil, catalog: nil, id: published.id)` line **unchanged** — the entry is what round 8 does *not* touch |
| `unit-app` | the catalog pane's own rows in both suites (**unchanged**) | The refactor is behaviour-preserving: `cellarTests` is green at the same distinct-id count, and the `detail-action-*` identifiers are asserted still declared once each in `PackageDetailView.swift` |

**Two mutations, both reversible and both `unit-app`.** (a) Re-adding `MutationMenu(center: operations, entry: …)`
to a pane must fail its `MutationMenu(` absence assertion; (b) adding a local `quietButton("Reinstall", …)`
to a pane must fail the verb-literal absence assertion. They prove different halves of "one component,
one place": the first that the pane did not keep the old surface, the second that it did not grow a copy
of the new one. Both are restored byte-identically and verified with `shasum -a 256 -c`.

### File Changes — round-9 delta (maintainer UI feedback, the shared icon tile on the tap rows)
...
| `cellar/Browse/BrowseView.swift` | **Untouched** | **0** | **DD-8** holds for the eighth round running |
| `Packages/CellarCore/**` | **Untouched** | **0** | No projection, no fact and no copy moves: round 9 is presentation only |
| `cellar/Activity/MutationMenu.swift` | **Untouched** | **0** | Fifth round running |
| `cellar.xcodeproj/project.pbxproj`, `cellarUITests/` | **Untouched** | **0** | No app-target file is created this round |

**Why no `.task` joins this file.** `BrowseView` carries `.task { await assets?.load() }`, and copying it
here would be the obvious move. It is forbidden and it is also pointless. Forbidden because PS8's
process-layer scenario scans this exact file for `.task`, `await ` and `async ` and `TapSearchView.swift`
must contain none of them (**DD-12**). Pointless because the only thing `assets` decides is
`isKnownIconToken`, which gates the CaskFlow rungs — and a cask published by a third-party tap is by
definition not in that manifest, so the gate answers `false` whether the catalog is loaded or not. The
surface hands the store through and lets the component ask; every other cask surface in the app already
calls `load()`, and it is idempotent.
