...
Homebrew's **per-package** trust grants: reading them with a compile-time-constant
`brew trust --json v1` probe, holding them in a store with the tap store's discipline, attributing each
entry to the tap that publishes it, accounting for every decoded entry including the namespaces Cellar
does not model, and surfacing the result **read-only**. Owned by `Packages/CellarCore` target
`BrewClient`, with app presentation in `cellar/Taps` and `cellar/Browse`.

Homebrew 6 grants trust at two independent granularities. `tap-management` owns a tap's **own** state,
read from `brew tap-info --installed --json`. This capability owns the second granularity, published
only by `brew trust --json v1`, which `tap-info` does not carry. It **shows** grants; it creates,
changes and removes none.

Acquisition, streaming and cancellation belong to `brew-execution` and are referenced here, never
restated. The shared mutation spine and its argv prohibition belong to `package-mutation` PM10; the
tap's own trust state, its badge and its controls belong to `tap-management` TM12 and TM13.
...
- **Established by change `m9-per-package-trust`** (archived `2026-08-24` —
  `openspec/changes/archive/2026-08-24-m9-per-package-trust/`), **ADDED-only delta — 8 requirements /
  33 scenarios**, 0 modified, 0 removed, 0 renamed, so `rules.archive`'s destructive-delta warning did
  not fire. The requirement and scenario bodies are promoted **byte-identical** from
  `openspec/changes/archive/2026-08-24-m9-per-package-trust/specs/package-trust/spec.md`; this file
  adds only the header prose, the `## Requirements` wrapper and this provenance section. Anchors PRD
  §3.7 (:108) and closes `2026-08-23-m7-tap-trust` follow-up item 1 (*a per-package trust surface*).
  Delivered in PR **#73**, merged `2026-08-24` at `ddb9661`.

- **The eight requirements are PT1–PT8, in file order.** This spec carries no `<!-- PTn -->` markers
  because the delta carried none, and promotion is byte-identical; only `tap-management` uses inline
  markers, and it inherited them from its own delta. The ordinals are fixed here so cross-capability
  references stay resolvable:

  | # | Requirement | Scenarios |
  |---|---|---|
  | **PT1** | Per-package grants are read from brew, into a three-valued state | 4 |
  | **PT2** | The grant store keeps the tap store's discipline and rides the taps domain | 6 |
  | **PT3** | Attribution rests on published qualified identity, never on splitting a string | 3 |
  | **PT4** | Every decoded entry is accounted for, including namespaces Cellar does not model | 5 |
  | **PT5** | One projection value supplies the count, and the marker states an exact fact | 4 |
  | **PT6** | Per-package copy is positive-only and is never a verdict | 4 |
  | **PT7** | This capability grants and revokes nothing | 3 |
  | **PT8** | Orphan grants are surfaced plainly, and the surface does not claim to close them | 4 |

  **30 `unit` + 3 `manual-evidence` = 33.** The `## Verification classes` table in the archived delta
  declares those classes; this spec keeps the inline `- Verification:` line on every scenario rather
  than restating the table, on the same convention `tap-management` and `package-mutation` adopted at
  `m7-tap-trust`.

- **Binding decisions consumed** (maintainer, 2026-08-24, Engram `sdd/m9-per-package-trust/scope-decisions`
  obs `#7759`), each recorded with what it **rejected**:
  - The surface is **read-only**. **Rejected: any per-package grant or revoke control.** The reason is
    mechanical, not stylistic: Homebrew treats *naming* a `/`-qualified package to its trust machinery
    (`trust.rb#explicitly_allowed?`) **as the grant**, so such a control needs a qualified token in
    argv — which `package-mutation` PM10 forbids everywhere, as an absence asserted by test. The
    deferral is gated on an **unmeasured** probe; see the deferrals entry below.
  - A **second read, source and store** owned by this capability. **Rejected: extending `tap-info`**
    to carry per-package grants — it does not publish them at all — and **rejected: reading
    `~/.homebrew/trust.json` (or any XDG-derived path) from disk**, because the location varies by
    configuration, the format is Homebrew's private business, and the standing project rule is to ask
    `brew` for what `brew` knows.
  - **No dedicated invalidation domain** (design decision **DD-3**, 2026-08-24). **Rejected: adding a
    `.packageTrust` `InvalidationScope` member** and declaring it across `MutationCommand`,
    `ServiceCommand`, `CleanupCommand`, `TapCommand`, `MutationGates.domain(for:)` and `RefreshDomain`.
    No Cellar command can mutate a per-package grant, so the domain would be **dead declaration
    surface on five command families** — a wide, shallow diff with no behaviour. The grant read rides
    the existing `.taps` domain instead (**DD-4**: refreshed concurrently via `async let`, with the
    **tap** read alone deciding the refresh outcome). This is what keeps `tap-management` TM12's
    amendment narrow: of the four things its single-source clause names, exactly **two** are taken — a
    second probe and a second store.
  - Unattributed grants get a **dedicated section in Taps**, and the `commands` namespace is **decoded
    and counted as "other"**. **Rejected: dropping either silently**, or counting them internally
    without a shipped surface. A documented exclusion is not a drop; a silent one is.
  - The count line lives on the tap **row and detail header**, both from **one** projection value
    (`TapGrantPresentation`), and package detail gets the **“Trusted individually”** marker beside its
    existing Tap fact (`package-detail` **PD8**). **Rejected: each surface deriving its own count**,
    and **rejected: any negative per-package copy** — no “0 trusted individually”, no “not individually
    trusted”, no muted marker. A package under a *trusted* tap is loadable with no individual entry at
    all, so absence from the report is not a fact about trust, and a zero line beside an `Untrusted`
    badge would read as the verdict `tap-management` TM11 prohibits outright.

- **Measured Homebrew 6 payload facts PT4 rests on** (Engram `#7764`, maintainer's Mac, 2026-08-24 —
  re-confirmed by ME1 during apply). They are cheap to lose and expensive to re-derive, and the
  captured payload **is** the apply fixture (`Tests/BrewClientTests/Fakes/TrustGrantFixture.swift`);
  it MUST NOT be re-invented:
  - The namespaces are **not disjoint**. `gentleman-programming/tap/engram` appears in **both**
    `formulae` and `casks`. Each occurrence is a distinct entry about a distinct package kind; neither
    deduplicates, displaces or masks the other.
  - A package name can carry **`@`** — `guria/tap/nehir@rc` is a real entry.
  - A `formulae` entry is **not always `owner/repo/name`**:
    `https://github.com/cloudmanic/spice-edit/spice-edit` was observed. Every positional split
    misreads it, which is why PT3 forbids splitting on `/` and requires published identity.
  - A namespace can be **present and empty** (`"commands": []`), which is a report of nothing and MUST
    stay distinguishable from a missing key.
  - The bare read is **side-effect free**: `trust.json` was byte-identical
    (`sha256:63ed7c9d…`, 553 bytes, mtime unchanged) before and after two reads, exit 0. Design Open
    Question 2 was **cleared by measurement** and MUST NOT be re-run as a gate.
  - Open Question 3, answered opportunistically: every entry in the real payload is **qualified**. An
    unqualified entry remains theoretical, so the section copy needs no extra sentence and no code
    change.

- **One `m7-tap-trust` belief was overturned by live measurement (ME2), and PT8 was amended to the
  measurement rather than the measurement explained away** (2026-08-24, Homebrew **6.0.18-182-ga963211**;
  Engram `#7775` rev 2; transcript filed at
  `openspec/changes/archive/2026-08-24-m9-per-package-trust/evidence/me2-transcript.txt`, not
  paraphrased). `brew untrust <tap>` **cascades** to that tap's per-package grants: untapping
  `guria/tap` from inside Cellar — which runs TM7's `brew untrust <tap>` behind an accepted removal —
  removed both `guria/tap/nehir` and `guria/tap/nehir@rc` from `casks`, ledger digest `63ed7c9d…` →
  `1150051f…`. Consequences, recorded exactly:
  - The **in-Cellar untap path leaves no orphan** and closes the dormant-grant hole **at package
    granularity too** — *stronger* than `m7-tap-trust` recorded. That change's **R7** claim, that
    per-package grants survive TM7's revocation, is **false for the in-Cellar path** and MUST NOT be
    carried forward unqualified.
  - Orphans arise only from an untap performed **outside** Cellar. The orphan on the measured Mac,
    `nkzw-tech/tap/codiff`, is a grant no Cellar action ever touched — which is exactly why PT8 forbids
    Cellar from claiming the cascade for grants it did not remove.
  - **PT8.3 was rewritten** to the outside-Cellar orphan it can actually evidence, and **PT8.4 was
    added** for the measured cascade. Both are `manual-evidence` and both are discharged by that one
    filed transcript. No code, no test and no other delta moved for the amendment (verified: the
    production diff across the amendment commit is empty).
  - **Restoration is honestly bounded.** Homebrew auto-updated mid-transcript, so the RESTORED payload
    was read on **6.0.19** while the cascade was measured on **6.0.18** — the finding predates the
    update and the restored report is content-identical to the fixture. Restoration ran **three**
    distinct commands (`brew tap guria/tap`, then `brew trust --cask guria/tap/nehir`, then
    `brew trust --cask guria/tap/nehir@rc`): **re-tapping alone re-armed nothing**, which corroborates
    PT8.4's closing clause instead of leaving it as an inference. Both points are annotated in the
    transcript's addendum. The screenshots referenced inside the transcript were viewed live but
    **were not archived**; the repository evidence for PT8.3 and PT8.4 is that transcript alone, and
    PT8.3's evidence clause names only the transcript for that reason.
  - **`proposal.md` and `explore.md` deliberately retain the superseded belief** as historical record.
    Rewriting them would hide that the belief was ever held.

- **What was deferred, and what gates each deferral:**
  - **Per-package grant/revoke controls** stay out of scope until the
    `brew untrust --formula|--cask <qualified>` probe answers whether the revocation **itself**
    registers a grant through `explicitly_allowed?` before removing it. The cascade finding above makes
    it *more* tempting to assume that revocation is safe, and it is precisely what was **not**
    measured. That probe gates any future mutation slice; it is not a prerequisite for anything this
    capability ships.
  - **`BrewfileDiff.isPresent` (R15)** is deliberately not in this change and writes no delta here.
  - **Trust state for official taps** remains out of reach: `tap-management` TM4 keeps official sources
    non-mutable, so no control could appear for them anyway.

- **Design-vs-spec reconciliations resolved in the spec's favour (B1–B5), recorded so a future reader
  does not mistake `design.md`'s superseded copy and three-category accounting for the shipped shape:**
  - **B1** — `TrustGrantSection.noneRecorded` renders the sentence
    “Homebrew records no packages trusted individually.”, not nothing; the section renders whenever it
    has a sentence or a non-empty group (PT6).
  - **B2** — `.unreported` renders exactly “This Homebrew does not report per-package trust.”,
    byte-compared by `theSectionCopyIsExactAndDistinguishesTheStates` (PT6).
  - **B3** — a non-empty orphan set renders exactly “Homebrew still records these grants. Cellar shows
    them; it does not remove them.” (PT8).
  - **B4** — the accounting is `UnattributedGrants { orphanTapGrants, unmatchedFormulae,
    unmatchedCasks, other, attributed, excluded }` with
    `total == attributed + excluded + surfacedCount`, asserted equal to `ledger.entryCount`. The
    design's three-category sketch is superseded by this five-way partition (PT4).
  - **B5** — `TrustGrantLedger.unmodelled: [String: [String]]` is filled by a dynamic-key container and
    its entries are counted as **other**; `declaredNamespaces` keeps present-and-empty distinguishable
    from absent (PT4).
  - **One further implementation deviation, reported rather than absorbed**: `TrustGrantLedger.isEmpty`
    is **not** package-scoped, though design DD-1 said it should ignore `taps`. Under B4 an *orphan tap
    grant* is its own accounted **and shown** category, so a package-scoped rule would collapse a
    ledger carrying only a `taps` entry to `.noGrants` and make that decoded entry disappear — the one
    thing PT4 forbids outright. The package-scoped notion survives where it belongs: as a count of zero
    *attributed* grants, which renders no count line (**DD-7**).

- **Cross-capability boundaries this capability does not own**, promoted in the same archive so they
  cannot drift: `tap-management` **TM12** (the tap's own state still comes from `tap-info` alone; the
  count line is additive and never touches the “Untrusted” badge), `package-mutation` **PM10** (no
  pre-launch gate on per-package grant state either; the argv prohibition **reaffirmed, not relaxed**,
  with `noPackagePositionEverCarriesAQualifiedToken` byte-identical and the C1 source-scanning ban list
  **extended** with the `TrustGrant` prefix and `grantsIndividually` — this change's one deliberate
  shipped-guard edit), and `package-detail` **PD8** (the marker beside the tap-of-origin fact, resolved
  by exact identity).
