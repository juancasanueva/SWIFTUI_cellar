...
Cellar's **second package source**: detecting the one `npm` binary the app reads, composing the
environment it runs under, reading the global inventory and the outdated report, the two npm mutation
commands and how their outcomes are classified, and the opt-in toggle that gates every npm surface.
Owned by `Packages/CellarCore` targets `BrewProcess` (locator, environment, detection state) and
`BrewClient` (payload sources, decoder, commands, store). Every requirement is behaviour of
`CellarCore` values; no SwiftUI view is named.

Scope is **globally installed npm packages** (`npm -g`) and nothing else. Per-project `node_modules`,
`npx`, installing from search, multiple simultaneous npm binaries, and npm in Cleanup, disk usage,
security scanning, Brewfile and Discover are all out of scope.

This capability does **not** own the inventory, the mutation spine, the activity log, history, the
menu bar or Health. Those stay with `installed-inventory`, `package-mutation`, `operation-activity`,
`installation-history`, `menu-bar` and `system-health`, each of which carries its own npm requirement.
What lives here is only what is npm-specific: which binary, under which environment, read how, and
turned on by whom.

Two invariants run through all ten requirements. **While the preference is off, nothing npm happens** —
no process is spawned, no state other than `disabled` is published, and the build is observably
identical to one without this capability. And **an npm that was not checked never reads as up to
date** — outdated state is tri-state (`fresh`, `notChecked`, `failed`), and only `fresh` may feed an
"everything is current" claim anywhere in the app.

Binding decisions taken before the delta was written (Engram `sdd/npm-package-source/decisions`): one
npm selected by priority rather than a set; the toggle defaults to **off**; brew and npm mutations are
**serialized** through one FIFO; the upgrade verb is `npm install -g <name>@latest` built from a
validated spec token; Health takes npm as **copy only**, not as a scored signal; and Settings shows
the selected npm's path, version and prefix and nothing more.
...
- **Established by change `npm-package-source`** (archived `2026-08-30` —
  `openspec/changes/archive/2026-08-30-npm-package-source/`), which closed PRD **M13 — npm package
  source** (`PRD.md` §7 :219-220). New capability: **ADDED-only — 10 requirements / 26 scenarios**, 0
  modified, 0 removed, 0 renamed, so `rules.archive`'s destructive-delta warning did not fire. The
  requirement bodies and the `## Requirements` wrapper are promoted **byte-identical** from
  `openspec/changes/archive/2026-08-30-npm-package-source/specs/npm-source/spec.md:18-325` (verified by
  an empty `diff` at archive); this file adds only the header prose above and this provenance section.
  The delta's own header carried a delta-local session-preflight block, which stayed in the archived
  delta and was not promoted.
- **The ten requirements are NS1–NS10, in file order.** This spec carries no `<!-- NSn -->` markers
  because the delta carried none and promotion is byte-identical. NS1 opt-in and off by default · NS2
  one npm by priority · NS3 the detected path/version/prefix triple · NS4 the environment · NS5
  `ls -g --json --depth=0` including an exit-1 payload · NS6 `outdated -g --json` and the tri-state ·
  NS7 the independent cadence · NS8 the two commands and their fixed argv · NS9 npm-native outcome
  classification · NS10 mutations gated on npm availability.
- **The locator priority is: configured path, `/opt/homebrew/bin/npm`, `/usr/local/bin/npm`, Volta,
  fnm default, nvm default, mise** (NS2). A configured path **never falls through** — it resolves to
  `invalid(notExecutable|notNpm)` or `configuredPathMissing` rather than silently selecting a different
  npm — and finding no npm anywhere is a soft `absent`, not an error.
- **Three safety properties are enforced structurally, not by review.** The shipped argv-interpolation
  scan globs `NpmCommand.swift` because it is a **top-level** command file (NS8); the environment is
  exactly PATH and HOME plus **seven** pins merged last, with no `HOMEBREW_*` (NS4); and
  `upToDateCopy` is a `String?` that is **absent** whenever a check has not completed, so "up to date"
  is unrepresentable rather than merely discouraged (NS6).
- **`brew-detection` was not widened for this.** npm detection is a sibling store sharing only the
  `ExecutableProbing` seam and the coalescing idiom; that non-coupling is BD6 there, proved at runtime
  by `NpmDetectionStoreTests.heldNpmEvaluationDoesNotCoupleToBrewDetection`.
- **Recorded count discrepancy.** The Engram spec mirror `#7972` describes this capability as
  "10 req / 24 sc". The canonical delta has **26** scenarios and the canonical file wins; verification
  counted 26 and the change's totals (24 requirements / 72 scenarios) were validated against the
  canonical files. The mirror was left uncorrected rather than rewritten after the fact, and the
  discrepancy is recorded as **W2** in the archived `verify-report.md` and `archive-report.md`.
- **No schema migration was performed and none was needed.** `"npm"` round-trips through the existing
  `kindRaw` column, so reverting `PackageKind.npm` leaves stored npm rows decoding to a nil identity
  and hidden, rather than crashing.
