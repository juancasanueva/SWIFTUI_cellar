...
- **Amended by change `m5-discover` (archived `2026-08-06`, PRD milestone **M5** "Pro-parity flows",
  slice 2 of 5 — the Discover tab)**: **2 MODIFIED** requirements replaced as whole blocks and
  **2 ADDED** requirements appended. 14 requirements / 51 scenarios → **16 requirements / 66
  scenarios**. Nothing was removed or renamed; the other twelve requirements are byte-identical, and
  every pre-existing scenario inside the two replaced requirements survives — all six byte-for-byte
  in "Slim persisted projection with a state sidecar", and in "Inspection data costs no new
  acquisition" one byte-for-byte with a single edited line in the other.
  - **"Slim persisted projection with a state sidecar"** (6 → 9 scenarios) **broadens** the version
    gate from "**both** persisted files — the snapshot and the state sidecar" to "**every file the
    catalog persists**", and extends a rule that previously only forbade adoption on another file's
    version so that it now also forbids *rejection* on another file's version. A file that adds **no**
    projection field MUST NOT require a snapshot `schemaVersion` increment: it records the version
    **its own owning schema declares**, which MAY be versioned independently, and is classified by
    exactly the same exact-in-both-directions gate against that schema. The independence is stated in
    **both** directions — a snapshot bump MUST NOT invalidate a still-matching sidecar, and a sidecar
    bump MUST NOT invalidate the snapshot. The footprint bound is now pinned to the snapshot **alone**
    and MUST NOT be re-based; every additional persisted file carries its own separately recorded size
    bound, likewise asserted by a test rather than by a comment.
  - **Independent sidecar versioning is user-protective, not stylistic.** What slice 1's hard
    constraint mandates is the gate *idiom* — exact in both directions, absence-not-error, never
    mutating the file it rejects — not the shared constant. Keying the roster and the arrivals log to
    `CatalogSnapshot.currentSchemaVersion` would let any unrelated projection bump silently erase a
    user's retained 30-day arrivals history, breaking a promise for a reason the user never caused.
    `DiscoverySchema` therefore declares its **own** `currentVersion = 1`.
  - **"Inspection data costs no new acquisition"** (2 → 3 scenarios) carries the **only relaxation in
    this merge**: the persisted-artefact enumeration widens from "exactly the snapshot and its state
    sidecar" to exactly four files — the snapshot, its state sidecar, the known-package roster and the
    dated arrivals log — "and nothing else". The zero-egress promise is unchanged and is extended to
    newness explicitly: the two new files are written from data the sync already holds. The same
    widening edits exactly one line of "The widened sync issues no additional request" — its closing
    `AND`, which enumerated the persisted files; every other line of that block is byte-identical.
  - **ADDED "A durable known-package roster records what this machine has already seen"** (6
    scenarios). Newness needs a durable seen-set because `CatalogSnapshotRevision` is process-local
    and restarts at 1 on every launch, so a diff keyed on it would report the entire catalog as new on
    the second launch. Missing, corrupt and version-mismatched all mean "seen nothing", which makes
    "a lost roster reports 16,214 arrivals" structurally unreachable rather than merely unlikely.
    **No newness field may be added to `CatalogPackage`** and the roster MUST NOT enter
    `CatalogSnapshot`: slice 1 closed with 2.4% of encoded-footprint headroom, which a persisted
    per-record first-seen field crosses at any plausible size. Roster read and write can never fail a
    sync.
  - **ADDED "The dated arrivals log retains thirty days and prunes itself"** (5 scenarios) — the small
    dated companion to the undated roster, so timestamps cost hundreds of entries rather than ~16k. A
    date is the moment of first observation **by this installation**, taken from the same time source
    that records `downloadedAt`, and MUST NOT be a publication or release date. An entry keeps its
    original date however often it is re-observed.
  - **`schemaVersion` stays 2 and `CatalogSnapshot` is untouched**, so no cache is invalidated in
    either direction, no re-download is triggered, and `CatalogFootprintTests` was re-run **unchanged**
    with a zero-line diff rather than re-based. Because the sidecars carry their own version, a
    reverted then re-applied slice **resumes** a user's existing 30-day history instead of reseeding it.
