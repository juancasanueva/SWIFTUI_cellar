...
- **Extended by change `m11-tap-search`** (archived `2026-08-25` —
  `openspec/changes/archive/2026-08-25-m11-tap-search/`), **1 ADDED (PS8), 0 modified, 0 removed, 0
  renamed** — **7 req / 19 sc → 8 req / 42 sc**. `rules.archive`'s destructive-delta warning did not
  fire. PS1–PS7 are **byte-identical**, verified at archive by byte-slicing against a pre-merge copy
  (empty diff), and PS8 was appended after "Index construction never runs on the main actor". The
  promoted block is byte-identical to
  `openspec/changes/archive/2026-08-25-m11-tap-search/specs/package-search/spec.md:132-728` (empty
  diff). Delivered in PR **#77**, merged `2026-08-25` at `2d96b1d`.
  - **A second result source, composed above the index — never in it.** `PackageSearchIndex` is built
    from the `CatalogSnapshot` alone, so every third-party tap package was unfindable even when the tap
    was installed and the package resident in `TapStore.inventory`. PS8 makes that inventory searchable
    from its **own** sidebar surface ("Search our taps"), fed by a `nonisolated`, `Sendable` projection
    over the resident inventory, with **zero** brew invocations, **no** snapshot record and **no** index
    entry. `openspec/specs/package-search/spec.md` PS1–PS7 therefore hold by construction rather than by
    argument, which is precisely why explore's Approach C (ingesting tap names into the index) was
    rejected: it would have falsified PD6 and made `Catalog` depend on `BrewProcess`.
  - **`Installed.`, `Not installed.` and `From your taps` were pinned by this change and then withdrawn
    by it.** `From your taps` named a Browse section that the 2026-08-25 scope change deleted before any
    of it shipped. `Installed.` / `Not installed.` were the row's state sentences until round 3's
    maintainer feedback replaced them with the **same shared status pill the catalog row draws** (and
    nothing at all for a not-installed row). None of the three reaches this file: PS8 is an ADDED
    requirement whose first appearance in `openspec/specs/**` is this merge, so the withdrawal needed no
    MODIFIED block and fired no destructive-delta warning. **`Installed.` and `Not installed.` remain
    live strings in TM5**, which governs the tap-detail rows, and round 6 put `Not installed.` back on
    the **name-only detail pane** — where no pill carries the fact — as TM5's exact string rather than a
    reworded one. Read the withdrawal as scoped to this surface's **rows**, never as a repository-wide
    retirement.
  - **The version prohibition is "no *published* version", and the adjective is load-bearing.** PS8 as
    first drafted forbade a version outright, because a tap declares none without the tap-source read
    TM5 forbids. Round 4 gave an installed hit an **offered version** derived from this machine's **own
    installed receipt**, under II4's existing outdated rule, so the prohibition was narrowed to the
    *published* value rather than dropped. The six-fact ceiling moved from five to six for the same
    reason. A future reader who reads the clause as "no version at all" will mis-scope it.
  - **The Outdated-control rationale was rewritten, and the rule survived the rewrite.** The original
    reason was "a tap hit carries no version, so the control could never change what is visible" (II8).
    Round 4 made that literally false for the installed-and-outdated minority, so the reason was
    replaced rather than quietly kept: an Outdated chip here would not filter the listing so much as
    **replace** it, collapsing every published package this machine does not have, and the catalog's own
    chip is defined over published versions this source still does not read. **Same rule, honest
    reason** — recorded because a rule whose stated justification has gone false is the kind of text
    that gets deleted by a later reader who checks the justification and not the rule.
  - **Nine rounds of maintainer UI feedback are compressed into one requirement.** In order: the
    surface moved out of Browse into its own section; the Installed pill replaced the state sentence
    (forcing `StatusPill`'s extraction from `PackageRow`, since Swift `private` is file-scoped and PS8
    asks for the *same* pill, not one that looks the same); the update pill followed; the installed
    verbs followed via the receipt handoff; a name-only pane arrived for unambiguous not-installed hits;
    colliding hits became routable to the catalog's own pane; both tap-backed panes adopted the catalog
    pane's Actions section; and the rows gained the shared leading icon tile. **Two 2026-08-24 decisions
    were reversed by the maintainer during apply** — "not-installed rows are non-selectable" (round 6)
    and "a colliding hit is non-selectable" (round 7) — and both reversals are recorded in the delta's
    decision table rather than rewritten away. The **only** inert row left is a duplicate `PackageID`
    among the emitted hits, which nothing can disambiguate.
  - **`package-mutation` was activated, not changed.** PM10 already mandates the bare, unqualified argv
    on the shared mutation spine and already forbids turning the untrusted-tap refusal into a pre-launch
    gate — including for a colliding token, where qualifying to disambiguate is exactly what it forbids.
    `sdd-spec` found no genuine gap and wrote no `package-mutation` delta. A source-scanning `unit-app`
    assertion, retargeted from the deleted Browse section to `TapSearchView`, keeps the absence
    enforced structurally.
  - **No `## Verification classes` table was promoted.** This file carries none today, so the delta's
    table stayed delta-local provenance and only the per-scenario inline `- Verification:` lines
    promoted with the requirement, following the `m7-tap-trust` precedent. Verified after the merge:
    `rg '^## Verification classes'` matches **zero** lines here.
