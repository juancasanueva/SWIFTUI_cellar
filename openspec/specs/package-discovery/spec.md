...
- Established by change `m5-discover` (archived `2026-08-06`, PRD milestone **M5** "Pro-parity
  flows", slice 2 of 5 — the Discover tab), ADDED-only delta — **6 requirements / 23 scenarios**,
  promoted from `openspec/changes/archive/2026-08-06-m5-discover/specs/package-discovery/spec.md`.
  This is the first main spec for the capability; nothing was modified, removed or renamed. This file
  adds the header, the `## Requirements` wrapper and this provenance section — the requirement and
  scenario bodies are byte-identical to the delta's.
- **This capability owns what Discover shows, never what it stores.** The known-package roster and
  the dated arrivals log that "New to you" reads are persisted by `catalog-sync` and specified there,
  not here. No SwiftUI view is specified anywhere in this file; every requirement is behaviour of
  `CellarCore` values.
- Traceability to the change's binding decisions (proposal Engram `#7492`, D1–D5 user-approved). Each
  names what was rejected, so a later change cannot reintroduce a rejected alternative as a fresh
  idea:
  - **D1** → "The curated list ships with the build and decodes tolerantly" and "A curated entry that
    no longer resolves is skipped and counted". The list is maintainer-authored, ships inside the
    build and is updated by shipping a release. **Rejected:** fetching or refreshing curated content
    over the network, which would break the zero-egress rule. A blurb is written in Cellar's voice
    and MUST NOT fall back to the package's own `desc`.
  - **D2** → "New to you means first observed by this machine, within 30 days". **Rejected:** the
    strictly-since-last-sync variant — simpler, but empty almost all the time, which would ship a
    section the user never sees populated.
  - **D3** → "Two separate ladders, fifty deep, absent is not zero". Formulae rank on
    `install-on-request` and casks on `cask-install`, which are different measurements.
    **Rejected:** a single merged ladder (it sorts incomparable numbers), an unbounded ranking, and
    including deprecated or disabled packages — recommending an abandoned package is worse than
    showing a shorter list.
  - **D4** (`.discover` sits between `.home` and `.browse`) is a shell/navigation decision with no
    `CellarCore` behaviour, so it is carried by the change's `tasks.md` and deliberately **not** by a
    requirement here. Recorded so a later reader does not read its absence as a gap.
  - **D5** → "Every section reports a typed state, and Discover never opens empty". **Rejected:** a
    tab-level empty screen, hiding the section entirely, and a spinner — the first-run state is a
    designed, explained state, not a pending one.
- **"Absent is not zero" is the spine of the ranking requirement.** `installCount365d` is `Int?`
  precisely so an unmeasured package can be excluded rather than ranked last. The shipped
  `PackageSearchIndex.defaultOrder` sorts absent counts **last** — right for a search result, wrong
  for a ranking, where "unmeasured" must not appear at all — and is deliberately not reused.
- **The honest phrasing of "new to you" is a requirement, not a UI note.** The explanatory copy is
  supplied by `CellarCore` and pinned by a source scan for `published`, `release date`,
  `added to Homebrew`, `new on Homebrew` and `recently added`, so a future view cannot quietly
  restate a local first observation as a publication date.
