...
- **Amended by change `npm-package-source`** (archived `2026-08-30` —
  ...
  source** (`PRD.md` §7 :219-220). **ADDED-only delta — 3 requirements / 11 scenarios**, 0 modified, 0
  removed, 0 renamed, so `rules.archive`'s destructive-delta warning did not fire. The requirement
  bodies are promoted **byte-identical** from
  `openspec/changes/archive/2026-08-30-npm-package-source/specs/installed-inventory/spec.md:14-125`
  (verified by an empty `diff` at archive); this file gained only that block and this bullet.
- **The added requirements are II17–II19 in file order.** II17 admits npm globals into the **one**
  inventory under identity `(npm, name)` with no migration, the catalog treating `.npm` as never
  present, and per-source failure isolation. II18 makes the Source filter (all/Homebrew/npm) and the
  NPM tag `CellarCore` projections that are unavailable with a typed reason — `disabled`, `absent`,
  `invalid` — and inert while unavailable. II19 makes the updates summary **per source**, so a clean
  brew set beside an unchecked or failed npm check is **not** "up to date".
- **II19 is the load-bearing one.** It is what stops the app from reporting silence as health: offline
  npm is stated, never rounded down to up to date, and the total stays
  `InstalledBrowse.outdatedCount(metadata:)` so this capability keeps owning outdated-ness for the
  menu bar, Home, Health and the sidebar badge that II16 already bound to it.
- **With the npm preference off, npm is omitted entirely** — no entry, no count, no filter option and
  no copy — so a build with the toggle off is observably identical to one without the capability.
