...
- Established by change `m6-sparkle-updates` (archived `2026-08-23`, PRD milestone **M6 "Ship"**,
  slice 3 of 3 — Sparkle 2 in-app updates), ADDED-only delta — **7 requirements / 31 scenarios**,
  promoted from
  `openspec/changes/archive/2026-08-23-m6-sparkle-updates/specs/app-updates/spec.md`.
  This is the first main spec for the capability and the project's **22nd**; nothing was modified,
  removed or renamed. This file adds the `# app-updates` header, the `## Verification classes`
  heading, the `## Requirements` wrapper and this provenance section — the requirement and scenario
  bodies are byte-identical to the delta's, and the capability-ownership paragraph, the class table
  and the design-owned paragraph are carried verbatim from it.
- **This capability owns what must be true of Cellar's update surface, never which framework
  provides it.** No requirement here names Sparkle, `SPUUpdater`, an SPM integration shape, a build
  setting, a workflow step or a project-file edit; those are *how* the contract is met and are
  archived with the change. `rules.specs`' "observable behaviour of `CellarCore` types without
  referencing SwiftUI views" clause **has a subject here**, unlike the previous slice — the change
  adds `CellarCore` types *and* two user surfaces — and is satisfied by naming every surface by what
  the user sees ("the Settings **Updates** group", "the app menu"), never by a SwiftUI type, a view
  file, or a property wrapper. The narrowing that replaces the clause is the per-scenario
  verification class above.
- **Execution status at promotion (2026-08-23).** The update surface is *implemented and
  runtime-proven*; the self-replacement it exists for is *not yet observed*. Per `verify-report`
  (Engram `#7690`, verdict `pass_with_warnings`, 0 blockers, 0 CRITICAL, validator-admitted
  `--requirements 10 --scenarios 35`): **19/19 `unit-core` and 11/11 `unit-app` scenarios executed
  and green** at `ab46446` — 216/216 `cellarTests` and 1,753/1,753 `CellarCore` tests, zero failures.
  The single `manual-evidence` scenario — *An installed build actually replaces itself from the
  published feed* — has **not been observed** and cannot be until a stable tag is published and the
  feed goes live; the feed URL returned `404` at verification, which is its correct pre-tag state.
  That is a deferred release checklist item recorded in the archive report, not an open defect in
  this spec.
- Traceability to the change's binding decisions (proposal Engram `#7682`, D1–D7 user-approved). Each
  names what was rejected, so a later change cannot reintroduce a rejected alternative as a fresh
  idea:
  - **D1** → "Automatic update checks stay off until the user asks…" and "An explicit update check is
    always reachable". An update check is network egress, and Cellar gates every other egress behind
    explicit consent, so the app's own persisted setting is the authority and is written to the
    updater at every launch. **Rejected:** the framework's own second-launch prompt asking the user
    to enable automatic checks — unstyled, and network consent obtained outside the gate every other
    egress passes through.
  - **D2** → no subject here; it lands on the `release-distribution` delta (the arm64 rewording for a
    universal vendored framework).
  - **D3** → the merge-preserves-history clause in "An appcast document is valid only if every field
    an update depends on is present and well formed". **Rejected:** a latest-only feed, accepted
    **only** as the degraded first run, and regenerating the whole feed from past assets.
  - **D4** → the prerelease clauses: a hyphenated version never appears in the feed, and a document
    containing one is rejected. **Rejected:** update channels and a channel picker — the framework's
    own documentation warns channels require every user to *already* run a channel-aware version, and
    an inert Settings row is forbidden ("no inert surface is rendered").
  - **D5 / D6** → "The feed the running app trusts is fixed inside the bundle" and "The updater
    reaches nothing but the updater": the feed URL and the 32-byte Ed25519 public key are merged into
    the bundle at build time from a partial property list held outside the synchronized app source
    group, with no runtime override of either. **Rejected:** carrying them as generated-Info.plist
    build settings — probe `U24` measured that the generator honours known keys only and drops them —
    and any user setting, environment variable, or network response that could substitute a feed or a
    key, because a key supplied at run time is a key an attacker can supply.
  - **D7** (attribution surface) is a process decision with no observable property of the update
    surface behind it, so it is carried by the change's `proposal.md` and deliberately **not** by a
    requirement here. Recorded so a later reader does not read its absence as a gap.
- **The public key is public by construction and the private key is a one-way door.** `SUPublicEDKey`
  ships inside every copy of the app and must never be handled as a secret; the corresponding private
  key must never exist in the repository in any form. Losing it permanently severs the update channel
  for every installed copy — rotation requires shipping a new public key through a non-update channel
  first — and re-landing after a revert **must reuse the same key**.
- **Contract shared with `release-distribution`.** The appcast items this spec validates are produced
  by that capability's publication path: the enclosure URL is the release run's own published
  `https` `github.com` asset URL, and the `26.0` minimum system version is the same deployment floor.
  Neither capability re-derives the other's facts.
- **Deliberately out of scope for v1**, recorded so their absence is not read as a gap: update
  channels, delta updates, a custom-styled update window (the framework's standard window is plain
  AppKit in the system appearance and mismatches the dark-only shell — accepted for v1, revisitable
  in v1.1), and release-notes presentation inside the update window (it crosses the existing
  release-notes consent gate and belongs to its own change).
