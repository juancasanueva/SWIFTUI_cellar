...
presented the package-removal disclosure and silently defeated the tap-add disclosure requirement
below — the `ConfirmationDisclosure.tapAdd` case, renamed from `tapTrust` on 2026-08-23 by
`m7-tap-trust` **D2**, because `brew tap` grants no trust on Homebrew 6.
...
- Established by change `m5-brewfile` (archived `2026-08-07`, PRD milestone **M5** "Pro-parity
  flows", slice 4 of 5 — Brewfile import & export, PRD §3.7 "Taps & Brewfile"), ADDED-only delta —
  **9 requirements / 38 scenarios**, promoted from
  `openspec/changes/archive/2026-08-07-m5-brewfile/specs/brewfile-management/spec.md`. This is the
  first main spec for the capability; nothing was modified, removed or renamed. This file adds the
  header, the `## Requirements` wrapper and this provenance section — the requirement and scenario
  bodies are byte-identical to the delta's.
- **The "Amended after design" paragraph above is carried verbatim from the delta.** The `design.md`
  it cites is archived at `openspec/changes/archive/2026-08-07-m5-brewfile/design.md` (Engram
  `#7523`) and stays authoritative wherever it is more specific than this file. The
  `package-mutation` rule it refers to was promoted into `openspec/specs/package-mutation/spec.md`
  in the same archive as PM1's disclosure clauses.
- **The PRD's literal implementation was refused, and that refusal is why this capability exists as
  written.** PRD §3.7 asks for a "diff preview via `brew bundle --file`". Probe U8 (2026-08-07,
  Homebrew 6.x, Engram `#7519`) established as **fact** that `brew bundle check --file <path>`
  *evaluates* the Brewfile's Ruby: a Brewfile whose only content was `File.write(".../marker.txt",
  ...)` left the marker on disk after that supposedly read-only preview. The product outcome is
  delivered; the implementation is not. The security invariants are therefore written as testable
  requirements, so a future "simplification" back toward `brew bundle check` fails a test rather
  than a review.
- Traceability to the change's binding decisions (proposal Engram `#7521`, D1–D6; D1–D3
  user-approved, D4–D6 settled in the proposal). Each names what was rejected, so a later change
  cannot reintroduce a rejected alternative as a fresh idea:
  - **D1** (import = Cellar parses the bytes) → "brew is never pointed at a file Cellar did not
    write", "An imported Brewfile is bytes, and nothing in it is ever evaluated", and "The diff is a
    pure offline projection, and costs no new acquisition". **Rejected:** A1, delegating the preview
    to `brew bundle check --file` — refuted by U8 above; and A3, an opt-in "run it anyway" escape
    hatch, which would have reintroduced the same code-execution path behind a checkbox.
  - **D2** (export = `brew bundle dump` into a Cellar-owned temp, then Cellar publishes) → "Export
    dumps to a Cellar-owned temporary path, and the subprocess never touches the user's disk" and
    "Publication is Cellar's atomic write to a destination chosen for that export". A fresh temp per
    export is what makes `--force` unable to reach a user file. **Rejected:** E1, pointing `dump`
    straight at the user's chosen destination, which hands `--force` a user path; E2 (Cellar
    composes the document from its own inventory) recorded as a fallback, not chosen, because the
    dump is brew's own answer and cannot drift from it.
  - **D3** (the import/export affordances live inside the existing Taps section, with no new
    `AppSection` case) is an app-side placement decision with no `CellarCore` behaviour, so it is
    carried by the change's `tasks.md` and deliberately **not** by a requirement here. Recorded so a
    later reader does not read its absence as a gap.
  - **D4** → "`trusted:` is parsed, surfaced, and confers nothing". A `trusted:` key is a claim by
    the *file's author*, who is exactly the party a trust decision must not be delegated to; a tap it
    names still raises the tap-add confirmation with identical text. (That case was named
    `ConfirmationDisclosure.tapTrust` when D4 was taken; `m7-tap-trust` **D2** renamed it to
    `.tapAdd` on 2026-08-23 — the rule is unchanged, only the case name.) The optional tap URL
    positional the grammar accepts comes from probe U9. **Rejected:** honouring it implicitly, and
    refusing files that carry it.
  - **D5** → "Every unaccepted construct is a counted skip with a named reason, and the count is
    zero, not absent". Unsupported kinds (`mas`, `vscode`, `whalebrew`, `go`, `cargo`, `uv`, `npm`,
    `krew`, `flatpak`, `winget`) are counted with their reason named. **Rejected:** silently
    dropping them, and refusing the whole file.
  - **D6** → the conditional clauses of "An imported Brewfile is bytes…" and of the skip
    requirement. A trailing Ruby `if`/`unless` is detected in order to *refuse* the line, never
    evaluated, so `brew "gnupg" if OS.mac?` parses identically on any host. **Rejected:** evaluating
    the condition, and whole-file refusal on encountering one.
- The three confirmed assumptions (Engram `#7520`) land as: a destination chosen per export → the
  destination clause of the publication requirement; missing entries pre-selected and present
  entries visible-but-unselectable → the selectability clauses of the diff requirement; skips never
  block an import → the final clause of the skip requirement.
- **`package-mutation` owns the spine, this capability owns the file.** A selection becomes existing
  typed mutations — `TapCommand.addTap` and `MutationCommand.install` — submitted through the shared
  mutation spine as `[AnyBrewMutation]`. No new `BrewMutating` conformer, no new invalidation
  domain, and no new confirmation vocabulary was introduced. `installed-inventory` and
  `tap-management` own the resident state the diff projects against; `brew-execution` owns process
  spawning and cancellation; `operation-activity` owns queue presentation. All are referenced here,
  never restated.
- **Tap-before-install ordering is load-bearing twice**, and the requirement says so: it is the
  correct execution order, *and* the shared confirmation gate derives a batch's disclosure from its
  first command, so the ordering is what makes a mixed batch present the tap-trust warning. It must
  hold even when the user selected the tap last.
- The archived delta spec is the verbatim audit trail.
- **Amended by change `m7-tap-trust`** (archived `2026-08-23` —
  `openspec/changes/archive/2026-08-23-m7-tap-trust/`), **2 MODIFIED, 0 added, 0 removed, 0 renamed**
  — **9 req / 38 sc → 9 req / 40 sc**. BF5 and BF7 carried 9 scenarios between them and were replaced by
  11. The other seven requirements are byte-identical to their prior text, and the destructive-delta
  warning did not fire.
  - **D2 (the rename)** — `ConfirmationDisclosure.tapTrust` → `.tapAdd(TapName)`, with a new
    `.tapTrustGrant(TapName)` for the real grant. Both requirements that named the case in prose moved
    with it. The rule each states is unchanged.
  - **D3 is the one new rule this delta carries**: a `/`-qualified package entry is applied by its
    **bare token**. Homebrew 6 treats naming a qualified package on the command line as a *per-package
    trust grant*, so forwarding the file's token verbatim would let a Brewfile's author grant trust on
    the importing user's Mac. **Rejected:** a carve-out for user-authored qualified names, and refusing
    qualified lines as counted skips. The strip happens in the plan, not in a gate:
    `BrewfilePlan.swift`'s install arm **does** change and is no longer a 0-line binding, while
    `MutationCommand.swift` is the 0-line binding now, because D3 adds no `/` gate to
    `MutationName.isSafe` or `PackageTarget.init?`.
  - **Homebrew 6 makes BF5's reasoning stronger, not weaker.** A `trusted:` key is a claim by the
    *file's author* — exactly the party a trust decision must not be delegated to — and now that trust
    is a genuine grantable capability, "confers nothing" explicitly includes "submits no trust command".
  - **BF7's ordering rule is now coupled to `package-mutation` PM1's amended derivation** ("the first
    submitted command that declares one **of its own**"). A Brewfile batch is always led by a tap add,
    which declares its own disclosure, so this capability's behaviour is unchanged — but the two
    sentences were promoted in the same archive so they cannot drift, exactly as `m5-brewfile` recorded
    for their first coupling.
  - **Two out-of-block `tapTrust` mentions were hand-edited at archive**, because a byte-correct
    whole-block promotion structurally cannot reach them: this file's header prose and the **D4**
    entry in the `m5-brewfile` provenance above. Both now name the tap-add disclosure and carry the
    dated rename note. **Two in-block mentions survive on purpose** — the `(Previously:)` rolling notes
    in BF5 and BF7 name the old case precisely to record that the rename happened. A `tapTrust` hit
    anywhere else in `openspec/specs/` would indicate a partial promotion; these four do not.
  ...
  - **Deferred, recorded as R15**: a qualified Brewfile entry keeps its qualified identity for
    *diffing*, so it always projects as "missing" even when the bare package is installed. Accepted for
    v1. A trust column in the Brewfile diff is deferred with it.
  - The archived delta spec is the verbatim audit trail.
