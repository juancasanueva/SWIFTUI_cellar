...
- Established by change `m6-tip-jar` (archived `2026-08-22`, PRD milestone **M6** "Ship", **slice 1**
  — the StoreKit 2 consumable tip jar), ADDED-only delta — **9 requirements / 40 scenarios**, promoted
  from `openspec/changes/archive/2026-08-22-m6-tip-jar/specs/tip-jar/spec.md`. This is the first main
  spec for the capability; nothing was modified, removed or renamed, and the change touched no other
  main spec. This file adds the header, the `## Requirements` wrapper and this provenance section —
  the requirement and scenario bodies are byte-identical to the delta's.
- **M6 remains OPEN.** This slice closed the tip jar only. Sparkle updates, a CI pipeline, the landing
  page and the rest of "Ship" are separate M6 slices, and none of them is specified here.
- **The header paragraphs above are carried verbatim from the delta.** The `probes.md` they cite is
  archived at `openspec/changes/archive/2026-08-22-m6-tip-jar/probes.md` (Engram `#7638`), and
  `design.md` is archived beside it (Engram `#7641`); the design stays authoritative wherever it is
  more specific than this file — most importantly the three-seam split
  (`TipCatalogLoading` / `TipPurchasing` / `TipGratitudeRecording`) and the single `"tips"` loop slot
  that runs hydrate → load → drain → observe.
- Traceability to the change's binding decisions (proposal Engram `#7639`; D1–D6 user-approved, Engram
  `#7637`). Each names what was rejected, so a later change cannot reintroduce a rejected alternative
  as a fresh idea:
  - **D1 — build now, state the risk** → "The tip surface exists only when the build can transact".
    A StoreKit consumable transacts only in a Mac App Store build, and Cellar ships with
    `ENABLE_APP_SANDBOX = NO` because it execs `brew`. The capability is therefore specified and fully
    tested without claiming it will ever transact. **Rejected:** waiting for the sandbox feasibility
    spike (U22) before building anything, and implying M6 ends with a transacting tip jar.
  - **D2 — one purchase call site** → "One purchase call site, and no new section". The Settings card
    beside `freeCard` is the only surface that may initiate a purchase, and `freeCard`'s copy is
    unchanged. **Rejected:** a `.support` `AppSection` — PRD :186's literal reading, but a whole page
    for one button exceeds "subtle" and costs a 22nd case across six exhaustive switches.
  - **D-E (Engram `#7642`) amends D2's literal "opens Settings" wording**: the About row is a
    **static** informational signpost, present only when the tip surface is available, and it carries
    no action of any kind. **Rejected:** a control that selects `AppSection.settings` — the root
    content view owns the selected section as private state and is a not-touched binding of this
    change, so navigation would have cost a diff the change was forbidden to make.
  - **D3 — availability derived from catalog emptiness** → "The tip surface exists only when the build
    can transact". Probe U20 measured `AppStore.canMakePayments` as always `true` locally, so the only
    clean "cannot transact" signal is `Product.products(for:)` returning an empty array.
    **Rejected:** an inert or disabled control; a `MAS_BUILD` compile flag, which is the nearest thing
    to the `#available` branches `rules.design` bans; and shipping external payment links alongside
    StoreKit, which App Review Guideline 3.1.1 forbids outright.
  - **D4 — one consumable, one tier** → the single product id `com.juancasanueva.cellar.tip` in "The
    tip catalog has three states". App Store Connect product ids are globally unique and permanent, so
    the id is declared once as a constant rather than repeated as a literal. **Rejected:** a tip ladder
    and `.quantity(n)`.
  - **D5 — one local boolean** → "The thank-you is one local boolean, and that is the whole record".
    **Rejected:** a session-only flag, which re-asks someone who already gave, and any dated, counted,
    identified or synced record — each would contradict the app's no-telemetry claim for no gain.
  - **D6** is a `PRD.md` text amendment (lines 9, 10, 186 and 234 rewritten in place, recording that
    StoreKit supersedes the earlier external-links ruling and that the Mac App Store ship path is
    unproven). It is carried by the change's `tasks.md`, not by a requirement here. Recorded so a later
    reader does not read its absence as a gap.
- **Two requirements are structural, and that is deliberate.** "One purchase call site, and no new
  section" and "StoreKit lives behind exactly one seam" constrain which file may reference which
  symbol rather than what a view renders, in the shipped `AppSecuritySources` / `OSVSource`
  composition-test idiom. They are assertable without rendering a SwiftUI view, which is what
  `rules.specs` asks for.
- **The zero-dependency `TipJar` target is the spine of requirement 9, not an optimisation.** With no
  dependencies the tip jar structurally cannot reach `brew`, the catalog, the network or SwiftData —
  a build-graph fact rather than a test assertion. One consequence was measured at apply time: a
  SwiftPM product with no dependents is not importable by the app target without an explicit link, so
  `project.pbxproj` gained **7 additive lines** (maintainer-approved, Engram `#7647`). The four other
  not-touched bindings held at exactly 0 lines.
- **Two scenarios are worded against mechanisms the design deliberately makes unreachable.** Scenario
  3.2 (an unverified transaction) and scenario 6.3 (network or process seams inside the
  zero-dependency target) are covered by executed rule-layer and structural evidence rather than by
  the mechanism their own text names: `SKTestSession` cannot mint an unverified transaction, and
  `TipJar` has no network or process seam to exercise. Amending that wording is a recorded follow-up;
  see `openspec/changes/archive/2026-08-22-m6-tip-jar/archive-report.md` §10.
