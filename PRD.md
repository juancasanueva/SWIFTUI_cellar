...
**M6 — Ship**
Menu bar extra; background checks + notifications (SMAppService); Settings; Spanish localization; accessibility pass; Sparkle integration; CI signing/notarization pipeline; self-hosted tap; landing page. (Tip jar was M6's first slice — built, then removed by decision; §6. The CI signing/notarization pipeline landed as M6's second slice — `RELEASING.md`. Sparkle 2 in-app updates landed as the third — `openspec/specs/app-updates/`. The self-hosted Homebrew tap landed as the fourth — now the shared `juancasanueva/homebrew-tap`, §6.) *Exit: 1.0 public release.*
